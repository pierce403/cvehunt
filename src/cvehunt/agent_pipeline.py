...
def _stage_prompt(
    ...
    if stage == "harness_builder":
        ...
        if cve_id == "CVE-2026-63030":
            runtime_contract += (
                ...
                "exactly schema='cvehunt.cve-2026-63030-target-acquisition/v1', cve_id, and variants: a JSON array of exactly "
                "two entries (names vulnerable and patched), each with exactly name, version, source_uri (the official "
    ...
    return (
        ...
        "For a completed stage outcome MUST NOT be \"none\". "
        f"For completed output payload MUST include these exact required keys: {required}. {payload_extension_rule}"
        f"{runtime_contract} "
        "artifacts entries contain exactly artifact_id, logical_path, classification. artifact_id values you declare MUST be unique and MUST NOT duplicate any artifact_id from the input manifest; do not redeclare input artifacts as your own outputs. Every model-created artifact MUST use model_input; "
