    ...
    def verify(
        ...
        if servable and proc.poll() is None:
            try:
                ...
                # Use the bare filename so `python3 poc.py` works under
                # cwd=run_dir; passing an already-absolute path plus cwd
                # resolves to cwd/path and breaks when cwd is itself a
                # relative run_dir path (it did, here, against GLM's PoC).
                comp = subprocess.run(
