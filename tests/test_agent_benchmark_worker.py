...
def bundle(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    public = {
        ...
        "evaluation_contract": {"sha256": "1" * 64},
        "result": {
            "schema": "cvehunt.dimensioned-result/v1",
            "implementation_status": "pre_conformance",
            "headline_eligible": False,
        },
        "stages": [{key: None for key in stage_fields}],
    }
    public["stages"][0].update(stage="collector", status="completed")
...
@pytest.mark.parametrize("mutation", ["hash", "headline", "scope", "collision"])
