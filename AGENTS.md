...
- Use `./contribute.sh <CVE-ID>` for the interactive contributor loop. It detects installed harness CLIs from `codex`, `gemini`, `claude`, `opencode`, and `pi`, prompts for the model, records attribution as `<harness>:<model>`, runs a persisted JSON workflow, and rebuilds the Vite dashboard unless `CVEHUNT_SKIP_BUILD=1` is set.
