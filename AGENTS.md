...
- Use `./contribute.sh <CVE-ID>` for the contributor loop. It detects installed harness CLIs from `codex`, `gemini`, `claude`, `opencode`, and `pi`, validates model names when the harness exposes a local catalog, runs an early target-isolation preflight, records attribution as `<harness>:<model>`, syncs missing Python or Node project dependencies when prompted, runs a persisted JSON workflow with `--execute-poc` enabled by default, invokes supported selected model CLIs afterward as read-only bounded evaluators, writes `model_attempt/`, `contribution_audit.{json,md}`, `contribution-interaction.log`, `contribute-output.log`, and `isolation-preflight.log` into the run directory, and rebuilds the Vite dashboard unless `CVEHUNT_SKIP_BUILD=1` is set. Each documented `CVEHUNT_*` environment override has an equivalent flag, including `--cve`, `--harness`, `--model`, `--skip-install`, `--skip-build`, `--skip-git`, `--dry-run`, `--execute-poc`, `--skip-execute-poc`, `--skip-model`, `--model-timeout`, and `--isolation-backend`.
...
- `./contribute.sh` now performs real external model invocation for supported harnesses (`pi`, `codex`, `gemini`, best-effort `claude`) after the deterministic Python workflow persists artifacts. The invocation is read-only/bounded, writes `model_attempt/prompt.md`, `transcript.txt`, `stderr.txt`, `response.md`, `command.txt`, and `metadata.json`, and does not let the model modify files directly. Set `CVEHUNT_SKIP_MODEL=1` / `--skip-model` to disable it. Set `CVEHUNT_EXECUTE_POC=0` / `--skip-execute-poc` to avoid the default local Docker PoC execution in contributor runs.
