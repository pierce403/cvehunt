...
- Use `./contribute.sh <CVE-ID>` for the interactive contributor loop. It detects installed harness CLIs from `codex`, `gemini`, `claude`, `opencode`, and `pi`, validates model names when the harness exposes a local catalog, runs an early target-isolation preflight, records attribution as `<harness>:<model>`, syncs missing Python or Node project dependencies when prompted, runs a persisted JSON workflow, writes `contribution_audit.{json,md}`, `contribution-interaction.log`, `contribute-output.log`, and `isolation-preflight.log` into the run directory, and rebuilds the Vite dashboard unless `CVEHUNT_SKIP_BUILD=1` is set. Each documented `CVEHUNT_*` environment override has an equivalent flag, including `--cve`, `--harness`, `--model`, `--skip-install`, `--skip-build`, `--skip-git`, `--dry-run`, `--execute-poc`, and `--isolation-backend`.
