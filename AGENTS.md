...
- Use `./contribute.sh <CVE-ID>` for the contributor loop. It detects installed harness CLIs from `codex`, `gemini`, `claude`, `opencode`, and `pi`, validates model names when the harness exposes a local catalog, runs an early target-isolation preflight, records attribution as `<harness>:<model>`, syncs missing Python or Node project dependencies when prompted, runs a persisted JSON workflow with `--execute-poc` enabled by default, invokes supported selected model CLIs afterward and extracts safety-checked model-authored artifacts under `model_attempt/`, writes `model_attempt/`, `contribution_audit.{json,md}`, `contribution-interaction.log`, `contribute-output.log`, and `isolation-preflight.log` into the run directory, and rebuilds the Vite dashboard unless `CVEHUNT_SKIP_BUILD=1` is set. Each documented `CVEHUNT_*` environment override has an equivalent flag, including `--cve`, `--harness`, `--model`, `--skip-install`, `--skip-build`, `--skip-git`, `--dry-run`, `--execute-poc`, `--skip-execute-poc`, `--skip-model`, `--model-timeout`, and `--isolation-backend`.
...
- Docker is installed in this environment (`docker --version` succeeded). `uv run cvehunt run` only builds/runs images when `--execute-poc` is set; `./contribute.sh` sets that by default unless `--skip-execute-poc` or `CVEHUNT_EXECUTE_POC=0` is used.
