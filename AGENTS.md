...
- Each persisted pipeline run should write `cve.json`, `trace.jsonl`, `pipeline_status.json`, `report.json`, and `report.md`.
...
- `uv run cvehunt run CVE-2025-55182 --persist --json` writes `cves/CVE-2025-55182/{cve.json,trace.jsonl,pipeline_status.json,report.json,report.md}` and currently reaches `defensive_signal_observed` at 0.82 confidence using synthetic fixture evidence.
