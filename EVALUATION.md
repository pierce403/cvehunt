...
For CVE-2026-63030 and CVE-2025-55182, production wiring requires a trusted, owner-pinned target policy before any run is reserved. The CVE-specific adapters validate the exact official WordPress or react-server-dom-webpack release identities and source hashes. React2Shell additionally uses a bounded archive container plan so the model can acquire and inspect npm packages while the trusted executor safely expands them for an offline build. Both host oracles accept only a nonce-bound target-filesystem effect. Remediation remains an independently derived dimension from fixed-target receipts matched to a capability-proving candidate and cannot alter primary exploit success.
